...
## 1. Overview
The Family AI Agent is a privacy-first, locally-hosted application designed to coordinate, plan, and remind family members about key recurring and seasonal events. It integrates with multiple Google Workspace accounts, uses Gemini for deep historical scheduling analysis, and sends summaries and proactive alerts via Slack.
...
*   **Notification Channel:** Slack Bot User (Official Slack WebClient SDK, highly reliable and stable).
...
*   **Storage & Broadcast:** These hierarchical profiles are saved to the database, and a detailed "Welcome Summary" is posted to the Slack channel.
